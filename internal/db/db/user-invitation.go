...
type UserInvitation struct {
	...
	Inviter       string `gorm:"column:inviter;type:varchar(255);index:idx_inviter"`         // 邀请人
	InviteCode    string `gorm:"column:invite_code;type:varchar(255);index:idx_invite_code"` // 被邀请码
	InvitePercent uint   `gorm:"column:invite_percent;type:int(11);not null;default:0"`      // 被邀请返佣
	
	InviteCount      uint `gorm:"column:invite_count;type:int(11);not null;default:0"`       // 邀请数量
	ValidInviteCount uint `gorm:"column:valid_invite_count;type:int(11);not null;default:0"` // 有效邀请数量（有交易用户
	
	LinkCount  uint `gorm:"column:link_count;type:int(11);not null;default:0"`     // 邀请链接数量
	MaxPercent uint `gorm:"column:max_percent;type:int(11);not null;default:2000"` // 20% 最大比例，百分位
	
	FirstTradeTxId string `gorm:"column:first_trade_tx_id;type:varchar(255);default:null;index:idx_first_trade_tx_id"`
